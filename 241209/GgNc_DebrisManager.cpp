...
void AGgNc_DebrisManager::BeginPlay()
{
	...
	if (!IsValid(DebrisBaseFX))
	{
		const TObjectPtr<UWorld> WorldPtr = GetWorld();
		if (IsValid(WorldPtr))
		{
			...
			const FVector Pos = FVector(0.0, 0.0, 0.0);
			const FRotator Rot = FRotator(0.0, 0.0, 0.0);
			DebrisBaseFX = WorldPtr->SpawnActor<ANiagaraActor>(Pos, Rot, SpawnParams);

			if (IsValid(DebrisBaseEffectTemplate))
			{
				DebrisBaseFX->GetNiagaraComponent()->SetAsset(DebrisBaseEffectTemplate);
			}
		}
	}
	...
	UnixTime = UKismetMathLibrary::Now().ToUnixTimestamp();
	FString Text = L"AGgNc_DebrisManager::BeginPlay()";
	TCHAR* PtrText = &Text[0];
	...
	LatentActionInfo.UUID = UnixTime;
	LatentActionInfo.ExecutionFunction = "DelayTest";
...
void AGgNc_DebrisManager::DelayTest() const noexcept
{
	const FString Text = FString::Printf(L"C++ Delay Func Test : %lld", UnixTime);
